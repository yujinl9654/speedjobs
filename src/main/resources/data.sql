...
-- -- POST
insert into posts (post_id, created_date, modified_date, content, title, view_count, user_id)
...
개발자 신입이 되려고 준비하는 학생입니다.
 ...
 MongoDB나 MySQL있는것 같은데 어떤 언어가 어떤 데이터베이스와 연동이 되는지 연결해주실수있나요?
  또 MongoDB는 SQL의 문법과 많이 다른가요?', ' Python', 22, 1),
(2, '2020-01-03 11:13:00', '2020-01-20 11:15:00', '둘의 차이가 스프링부트가 설정이 더 쉽다 이게 가장 큰 차이인가요? gradle 과 메이븐의 차이는 별로없는건가요.... 스프링 개발자가 스프링부트를 공부할때 러닝커브는 어느정도 될까요?', '스프링과 스프링부트의 차이', 123, 3),
(3, '2020-01-03 11:13:00', '2020-01-20 11:15:00', '계속 들어만 봐왔어서 어떤 차이점이 있는지 궁금해요.', 'iBatis와 MyBatis의 차이점', 43, 3),
...
내일 면접있는데 이렇게 뿔난식으로 대답할까봐 걱정됩니다', '인터페이스의 장점에 대해 모르겠습니다. 납득좀 시켜주세요', 2, 5),
(5, '2020-01-03 11:13:00', '2020-01-20 11:15:00', '로그나 구글 검색해보면 두루뭉실하게 겉핥기처럼 간단하게 설명만 나와있고, 구체적인 예시란게 없어서 공부해보려는 제가 어떻게 시작해야할 지 막막하네요.
 도움이될만한 포스트나 예시가 있을까요? 감사합니다.', 'rest api란게 도대체 뭔가요?', 3, 9),
 ...
 클로저란 내부함수가 외부함수의 지역변수에 접근 할 수 있고, 외부함수는 외부함수의 지역변수를 사용하는 내부함수가 소멸될 때까지 소멸되지 않는 특성을 의미한다.
  자신의 생성된 시점의 환경을 기억하는 함수
   렉시컬 스코프 : 함수가 중첩될 때 parser가 변수의 이름을 해석하는 방법
    ...
    클로져를 사용하면 가비지 컬렉션 대상이 되어야 할 객체가 메모리 상에 남아 있게 되므로, 클로져를 남발하면 메모리를 과다로 잡아 먹을 수 있기 때문이다.', '[Closure] 클로져란 무엇인가?', 22, 20),
(7, '2020-01-03 11:13:00', '2020-01-20 11:15:00', '현재 톰캣으로 90 포트로 돌고 있는 서비스가 있는데요~ 이번에 ssl로 변경 하게 되었습니다.
 그래서 server.xml의 <Connector port> 부분에서  redirectPort="443" 으로 해줬는데요. 기준의 윈도우 방화벽 인바운드가 90 포트로 되어있는데 이거에 443포트를 추가 해줘야 하는건가요?
  아니면 인바운드에  90 포트는 지우고 443 포트만 추가 해줘야 하는건가요?', '초보개발자 ssl 질문이 있습니다.', 432, 13),
(8, '2020-01-03 11:13:00', '2020-01-20 11:15:00', '자바 정규식에 대해 알고싶어요.', '자바 정규식', 42, 14);
